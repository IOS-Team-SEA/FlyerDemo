...
//       let image2 = convertTextToImageCTNew(attributedString: attributedString, refFrameSize: refSize, textBounds: mainBound)
    ...
    func drawImageCoreText(textProperties: TextProperties, refSize:CGSize , attributedString : NSMutableAttributedString, maxWidth : CGFloat = .infinity, maxHeight:CGFloat = .infinity , isHeightConstraint : Bool = false,paddingInPerc : CGFloat = 0 ) -> UIImage? {
...
//       let image2 = convertTextToImageCTNew(attributedString: attributedString, refFrameSize: refSize, textBounds: mainBound)
    ...
    func extractText(from frame: CTFrame, attributedString: NSAttributedString) -> String {
        var extractedText = ""
...
//        // Now scale the output image
//         if let originalImage = textImage {
//             let scaledImageSize = CGSize(width: originalImage.mySize.width * scaleFactor, height: originalImage.mySize.height * scaleFactor)
...
//    func convertTextToImageCTNew(attributedString:NSAttributedString ,refFrameSize:CGSize, textBounds : CGRect ,xAlign : HTextGravity = .Center, bgColor : UIColor = .clear, maxWidth: CGFloat , maxHeight:CGFloat = .infinity) -> UIImage?{
//        
//        
//        
//        UIGraphicsBeginImageContext(refFrameSize)
...
//        
//        bgColor.setFill()
//        ctx.fill(CGRect(origin: .zero, size: scaledFrameSize))
//        
//        ctx.textMatrix = .identity
//        ctx.translateBy(x: 0, y: scaledFrameSize.height)
//        ctx.scaleBy(x: 1.0, y: -1.0)
//        
//        let frmeSetter = attributedString.framesetter()
//        let frameSize = frmeSetter.frameSize(suggested: CGSize(width: maxWidth * scaleFactor, height: maxHeight * scaleFactor))
//        let frame = attributedString.framesetter().createFrame(CGRect(origin: .zero, size: frameSize))
//        
//        var xOffset: CGFloat = 0
//        var yOffset: CGFloat = 0
...
//        switch xAlign {
//        case .Center:
//            xOffset = ((scaledFrameSize.width / 2) - (scaledTextBounds.origin.x + (scaledTextBounds.width / 2)))
//        case .Left:
//            xOffset = -scaledTextBounds.origin.x
//        case .Right:
//            xOffset = scaledFrameSize.width - (scaledTextBounds.origin.x + scaledTextBounds.width)
//        }
//        
//        switch CTAlignmentY.center {
//        case .center:
//            yOffset = ((scaledFrameSize.height / 2) - (scaledTextBounds.origin.y + (scaledTextBounds.height / 2)))
//        case .bottom:
//            yOffset = -scaledTextBounds.origin.y
//        case .top:
//            yOffset = scaledFrameSize.height - (scaledTextBounds.origin.y + scaledTextBounds.height)
//        }
//        
//        if attributedString.string != "" {
//            UIColor.orange.setFill()
//            ctx.fill(CGRect(origin: CGPoint.zero, size: scaledFrameSize))
//            
//            ctx.translateBy(x: xOffset, y: yOffset)
//            frame.draw(in: ctx)
//            ctx.setStrokeColor(UIColor.white.cgColor)
//            ctx.stroke(scaledTextBounds)
//            
//        } else {
//            UIColor.orange.setFill()
//            ctx.fill(CGRect(origin: CGPoint.zero, size: scaledTextBounds.size))
//        }
//        
//        let textImage = UIGraphicsGetImageFromCurrentImageContext()
//        UIGraphicsEndImageContext()
...
//    func getExactFontSize(with marginInPerc : CGFloat = 15, userText : String , fontName : String , refSize : CGSize, isWidthConstraint : Bool = false , isHeightConstraint : Bool = false) -> CGFloat {
//        
//        let margin = ( marginInPerc / 100 ) * (min(refSize.width,refSize.height)) // extra padding to give fontSize breathing space
//        var hi:Int = 1000
//        var lo: Int = 1
...
//        repeat {
//           // autoreleasepool {
//           let middleFont = UIFont(name: fontName, size: CGFloat(mid))!
//            
////            let exactBounds = getExactBounds(refSize: refSize, font: middleFont, userText: userText , isWidthConstraint: isWidthConstraint, isHeightConstraint: isHeightConstraint ,paddingInPerc: marginInPerc) // Get Exact Bounds With Current Font Size
////
////            if exactBounds == CGRect.zero { // Font Size Too Large : reduce Font Size
...
//    func getIdealSize(for text : String , font : UIFont , refSize : CGSize , isWidthConstraint : Bool = false, isHeightConstraint : Bool = false, widthPadding:CGFloat = 5) -> CGSize{
//       var currentText = text
//        if currentText == " " {
//         //   currentText = "-"
//        }
//        let frame = getCTFrame(refSize: refSize, font: font, userText: currentText)
//        let standardLineHeight = getLineHeight(frame: frame)
//        let lineCount = frame.lines().count
//        
//        let bestSize = getExactBounds(refSize: refSize, font: font, userText: currentText , isWidthConstraint : isWidthConstraint , isHeightConstraint: isHeightConstraint , paddingInPerc: widthPadding)
//        
//        return CGSize(width: bestSize.width, height: standardLineHeight * CGFloat(lineCount))
//        
//    }
...
////    func calculateBestFrame(refSize:CGSize , font: UIFont , userText : String) -> CGRect{
////
//////        var characters = Array(userText)
...
//////            }
//////        }
////       // let userText = userText.replacingOccurrences(of: " ", with: "-")
//////        var mainBound = CGRect.zero
//////        var minX = CGFloat.zero
//////        var maxX = CGFloat.zero
//////        var minY = CGFloat.zero
...
//////           ]
//////        let attributedString = NSAttributedString(string: userText,attributes: attributes)
//////        let frame = attributedString.framesetter().createFrame(CGRect(origin: .zero, size: refSize))
//////        frame.draw(in: ctx)
////////        ctx.stroke(colr: .purple)
////////
//////        var origins = frame.lineOrigins()
//////        var lines = frame.lines()
...
//////            var lineBox = line.imageBounds(in: ctx)
//////
//////            lineBox.origin = origins[i]
//////
//////            lineBounds.append(lineBox)
//////
//////
...
//////        //  run.draw(in: ctx)
////////            print("ORIGINES : ",run.baseOrigins())
////////            print("IMAGE BOUNDS: ",run.imageBounds(context: ctx))
////////            print("POSITION GLYPH: ",run.glyphPositions())
////////            print("other: ",run.typographicBounds())
//////
//////            let font = run.font
//////            let glyphPositions = run.glyphPositions()
//////                            let glyphs = run.glyphs()
//////            let glyphsBoundingRects =  font.boundingRects(of: glyphs)
//////                var leftCharacterOrigin : CGPoint = CGPoint.zero
//////
//////                ctx.setStrokeColor(UIColor.black.cgColor)
//////                ctx.setLineWidth(1.0)
//////                // LOOP FOR CHARACTERS
...
//////
//////                               var box = gRect
//////                box.origin.x +=  point.x + origins[i].x + CGRect(origin: .zero, size: refSize).origin.x
//////                box.origin.y +=  point.y + origins[i].y + CGRect(origin: .zero, size: refSize).origin.y
//////                let max = min(leftCharacterOrigin.y,box.origin.y)
//////                leftCharacterOrigin.y = max == 0 ? box.origin.y : max//-  box.height
//////                print("Line Y : ",leftCharacterOrigin.y)
//////
//////                leftCharacterOrigin.x = leftCharacterOrigin.x == 0 ? box.origin.x : min(box.origin.x,leftCharacterOrigin.x) //box.origin.x + point.x + CGRect(origin: .zero, size: size).origin.x
//////                   // leftCharacterOrigin.y = box.origin.y + point.y + CGRect(origin: .zero, size: size).origin.y
//////
//////               // }
//////
//////
...
//////
//////
//////                lineBounds[i].origin.x =  leftCharacterOrigin.x
//////                lineBounds[i].origin.y =   leftCharacterOrigin.y
//////
////////                ctx.setStrokeColor(UIColor.blue.cgColor)
////////                ctx.setLineWidth(0.7)
////////                ctx.stroke(lineBounds[i])
//////
//////        } // FOR WORDS
//////
//////            mainBound.origin.x = mainBound.origin.x == 0 ?  lineBounds[i].origin.x : min(lineBounds[i].origin.x,mainBound.origin.x)
//////            mainBound.origin.y = mainBound.origin.y == 0 ?  lineBounds[i].origin.y : min(lineBounds[i].origin.y,mainBound.origin.y)
//////            minX = mainBound.origin.x
//////            maxX = maxX <= minX ? lineBounds[i].maxX : max(maxX,lineBounds[i].maxX)
//////
//////            mainBound.size.width = maxX - minX//mainBound.size.width == 0 ?  lineBounds[i].size.width : max(lineBounds[i].size.width,mainBound.size.width)
//////            minY = mainBound.origin.y
//////            maxY = maxY <= minY ? lineBounds[i].maxY : max(maxY,lineBounds[i].maxY)
//////            mainBound.size.height = maxY - minY//mainBound.size.height == 0 ?  mainBound.origin.y+(mainBound.maxY - mainBound.minX): max(lineBounds[i].maxY,mainBound.origin.y+(mainBound.maxY - mainBound.minX))
//////
//////
//////        }// FOR LINES
...
//////
//////        ctx.stroke(mainBound)
//////        print("Canvas Size : ", refSize , "\n\nImageBounds : ",mainBound)
//////
//////         return mainBound
////
////
...
////             let attributedString = NSAttributedString(string: userText,attributes: attributes)
////        let frmeSetter = attributedString.framesetter()
////        let frameSize = frmeSetter.frameSize(suggested: CGSize(width: CGFloat.infinity, height: .infinity))
////             let frame = attributedString.framesetter().createFrame(CGRect(origin: .zero, size: frameSize))
////
//////             ctx.stroke(colr: .purple)
//////
...
////
////             var image = UIGraphicsGetImageFromCurrentImageContext()
////        var maxYTemp = CGFloat.zero
////        var maxXTemp = CGFloat.zero
////
////             for i in 0 ..< origins.count {
////
////             let line = lines[i]
////                print("Line No \(i+1) Trailing Space : " , CTLineGetTrailingWhitespaceWidth(line))
////                 ctx.textPosition = .zero
////                 var lineBox = line.imageBounds(in: ctx)
////
////                 lineBox.origin = origins[i]
...
////
////                 // Loop For LINE
////                for r in 0...line.glyphRuns().count - 1 {
////                    let run = line.glyphRuns()[r]
////
////             //  run.draw(in: ctx)
////     //            print("ORIGINES : ",run.baseOrigins())
////     //            print("IMAGE BOUNDS: ",run.imageBounds(context: ctx))
...
////
////                                    var box = gRect
////                     box.origin.x +=  point.x + origins[i].x + CGRect(origin: .zero, size: size).origin.x
////                     box.origin.y +=  point.y + origins[i].y + CGRect(origin: .zero, size: size).origin.y
////                     let minY = min(leftCharacterOrigin.y,box.origin.y)
////                     leftCharacterOrigin.y = (k == 0 && r == 0) ? box.origin.y : minY//-  box.height
////                     print("Line Y : ",leftCharacterOrigin.y)
////                    maxYTemp = (k == 0 && r == 0) ? box.maxY : max(maxYTemp,box.maxY)
////
////                     leftCharacterOrigin.x = (k == 0 && r == 0) ? box.origin.x : min(box.origin.x,leftCharacterOrigin.x)
////                    maxXTemp = (k == 0 && r == 0) ? box.maxX : max(maxXTemp,box.maxX)
////                    //box.origin.x + point.x + CGRect(origin: .zero, size: size).origin.x
////                        // leftCharacterOrigin.y = box.origin.y + point.y + CGRect(origin: .zero, size: size).origin.y
////                   // print("Line X : ",leftCharacterOrigin.x)
////
////                    // }
////
////
...
////             } // FOR WORDS
////
////                 mainBound.origin.x = i == 0 ?  lineBounds[i].origin.x : min(lineBounds[i].origin.x,mainBound.origin.x)
////                 mainBound.origin.y = i == 0 ?  lineBounds[i].origin.y : min(lineBounds[i].origin.y,mainBound.origin.y)
////                 minX = mainBound.origin.x
////              //   maxX = maxX <= minX ? lineBounds[i].maxX : max(maxX,lineBounds[i].maxX)
////                maxX = maxX <= minX ? maxXTemp : max(maxX,maxXTemp)
////
////                 mainBound.size.width = maxX - minX
////
////                ///mainBound.size.width == 0 ?  lineBounds[i].size.width : max(lineBounds[i].size.width,mainBound.size.width)
////                 minY = mainBound.origin.y
////                 //maxY = maxY <= minY ? lineBounds[i].maxY : max(maxY,lineBounds[i].maxY)
////                maxY = maxY <= minY ? maxYTemp : max(maxY,maxYTemp)
////
////                 mainBound.size.height = maxY - minY//mainBound.size.height == 0 ?  mainBound.origin.y+(mainBound.maxY - mainBound.minX): max(lineBounds[i].maxY,mainBound.origin.y+(mainBound.maxY - mainBound.minX))
////
////
...
////
////
////       // ctx.translateBy(x: -mainBound.origin.x, y: 0)
////        let zeroOffset = -mainBound.origin.x
////        let xOffset = ((refSize.width/2) - (mainBound.origin.x + (mainBound.width/2)))
...
////        ctx.translateBy(x: (mainBound.width)/2, y: 0)
////            // lets Assume Alignmet Is In Left
//////            mainBound.origin.x = 0
////
...
//    
//    
////    func calculateBestFontSize(with marginMultipier : CGFloat = 0.2, userText : String , fontName : String , refSize : CGSize, keepWidthInfinity : Bool = false , keepHeightInfinity : Bool = false) -> CGFloat{
////
////        let marginWidth = refSize.width * marginMultipier
////        let marginHeight = refSize.height * marginMultipier
////
////        let cal = CTCalculator()
////
////        var hi:Int = 1000
////        var lo: Int = 1
////        var mid:Int = (Int)(hi + lo)/2
...
////           let middleFont = UIFont(name: fontName, size: CGFloat(mid))!
////
////           let attributes = [NSAttributedString.Key.font : middleFont]
////
////           let attributedString = NSAttributedString(string: userText, attributes: attributes)
////
////           var size = refSize
////
////          // if isWidthFlexible {
////              // size.width = CGFloat(MAXFLOAT)
////          // }
////          // if isheightFlexible {
////              // size.height = CGFloat(MAXFLOAT)
////          // }
////
////           let options: NSStringDrawingOptions = [.usesLineFragmentOrigin, .usesFontLeading]
////          // let textSize = attributedString.boundingRect(with: size, options: options, context: nil)
////          // let textSize2 = text.size(withAttributes: attributes)
////          // if isWidthFlexible && isheightFlexible {
////            let textSize = calculateBestFrame(refSize: refSize, font: middleFont, userText: userText)
////           // let image = cal.convertTextToImageCT(font: middleFont, userText: userText, textColor: .black, refFrameSize: refSize, textBounds: textSize)
////            if textSize == CGRect.zero {
////                hi = mid - 1
////            } else
////            if textSize.height + marginWidth <= refSize.height  && textSize.width + marginHeight <= refSize.width{
////                lo = mid + 1
////            } else{
////                hi = mid - 1
////            }
////           //}
//////           else if isWidthFlexible && isheightFlexible == false{
//////
//////                   if textSize.height + 5 <= constraintSize.height {
//////                        lo = mid + 1
//////                    } else{
//////                        hi = mid - 1
//////                    }
//////           }else if   isheightFlexible && isWidthFlexible == false{
//////                   if textSize.width + 5 <= constraintSize.width {
//////                        lo = mid + 1
//////                    } else{
//////                        hi = mid - 1
//////                    }
...
////
////            mid = (Int)(hi + lo)/2
////           // completion(  CGFloat(mid))
////
////        } while lo <= hi
////       // let f = calculateOptimalFontSize(textLength: CGFloat(text.count+2) , boundingBox: CGRect(origin: .zero, size: bounds))
////        let textSize = calculateBestFrame(refSize: refSize, font: UIFont(name: fontName, size: CGFloat(mid))!, userText: userText)
////
////        let image = cal.convertTextToImageCT(font: UIFont(name: fontName, size: CGFloat(mid))!, userText: userText, textColor: .black, refFrameSize: refSize, textBounds: textSize)
////
////        return CGFloat(mid)
////    }
//    
...
//    }
////    func convertTextToImageCT(font: UIFont, userText : String , textColor : UIColor ,refFrameSize : CGSize , textBounds : CGRect ,xAlign : CTAlignmentX = .center, yAlign : CTAlignmentY = .center , bgColor : UIColor = .clear) -> UIImage?{
//        func convertTextToImageCT( attributedString : NSMutableAttributedString ,refFrameSize : CGSize , userProperties:TextProperties ) -> UIImage?{
//
//            let textBounds =  attributedString.string.bestBoundingRect(properties: userProperties, ignoreSpacing: false) 
//            
//            let textBounds2 = CTCalculator3().getExactBounds(refSize: refFrameSize, font: UIFont(name: userProperties.fontName, size: userProperties.fontSize)!, userText: attributedString.string,isWidthConstraint: true)
//        
////        let paragraphStyle = NSMutableParagraphStyle()
////           paragraphStyle.alignment = .center
////           let attributes = [
...
//            xOffset = -x
//        case .right:
//            xOffset = refFrameSize.width - (x + (textBounds2.width));
//            case .justified:
//                xOffset = ((refFrameSize.width/2) - (x + (textBounds2.width/2)))
//            case .natural:
//                xOffset = ((refFrameSize.width/2) - (x + (textBounds2.width/2)))
...
//        
//    }
//    func getCTFrame(refSize:CGSize , font: UIFont , userText : String, isWidthConstraint:Bool = false , isHeightConstraint : Bool = false) -> CTFrame {
//        var center = CGPoint.zero
//        let paragraphStyle = NSMutableParagraphStyle()
//           paragraphStyle.alignment = .center
//        let attributes = [
...
//        let attributedString = NSAttributedString(string: userText,attributes: attributes)
//       let frmeSetter = attributedString.framesetter()
//        let frameSize = frmeSetter.frameSize(suggested: CGSize(width: isWidthConstraint ? refSize.width : CGFloat.infinity, height: isHeightConstraint ? CGFloat.infinity : .infinity))
//        let frame = attributedString.framesetter().createFrame(CGRect(origin: .zero, size: frameSize))
//        return frame
//    }
//    func getLineHeight(frame:CTFrame)->CGFloat{
...
//    }
//    
//    func getExactBounds(refSize:CGSize , font: UIFont , userText : String , isWidthConstraint:Bool = false , isHeightConstraint : Bool = false,paddingInPerc : CGFloat = 15 ) -> CGRect {
//        
//        var mainBound = CGRect.zero
//        var minX = CGFloat.zero
//        var maxX = CGFloat.zero
...
//        
//        // convert frame to CTFrame For further CoreText Based Calculation ( lines,run and glyphs )
//        let frame = getCTFrame(refSize: refSize, font: font, userText: userText ,isWidthConstraint: isWidthConstraint , isHeightConstraint: isWidthConstraint)
//        frame.draw(in: ctx)
//        // get Origins Of Every Line
//        var lineOrigins = frame.lineOrigins()
//        
...
//        return mainBound
//    }
//    func getExactFontSize3(with marginInPerc : CGFloat = 15, userText : String , properties : TextProperties, refSize : CGSize, isWidthConstraint : Bool = false , isHeightConstraint : Bool = false) -> CGFloat {
//        
//        let margin = ( marginInPerc / 100 ) * (min(refSize.width,refSize.height)) // extra padding to give fontSize breathing space
//        var hi:Int = 1000
//        var lo: Int = 1
...
//            properties1.fontSize = CGFloat(mid)
//            autoreleasepool {
//                let exactBounds = userText.bestBoundingRect(properties: properties1 , ignoreSpacing: false)
//                
//      //  let exactBounds = getExactBounds(refSize: refSize, font: middleFont, userText: userText , isWidthConstraint: isWidthConstraint, isHeightConstraint: isHeightConstraint ,paddingInPerc: marginInPerc) // Get Exact Bounds With Current Font Size
//        
//                if exactBounds == CGSize.zero { // Font Size Too Large : reduce Font Size
...
//        return CGFloat(mid)
//    }
//    func getExactFontSize2(with marginInPerc : CGFloat = 15, userText : String , fontName : String , refSize : CGSize, isWidthConstraint : Bool = false , isHeightConstraint : Bool = false) -> CGFloat {
//        
//        let margin = ( marginInPerc / 100 ) * (min(refSize.width,refSize.height)) // extra padding to give fontSize breathing space
//        var hi:Int = 1000
//        var lo: Int = 1
