...
func getRealFont(nameOfFont :String) -> String {
    var path:String?
    if let tpath = Bundle.main.path(forResource: nameOfFont, ofType: nil) {
        path = tpath
    }else if let fPath = ImageDownloadManager.loadFontFromDocumentsDirectory(fontName: nameOfFont){
        path = fPath
    }else{
    ...
    if let fontDataProvider = fontDataProvider{
        ...
        let documentsDirectory = (AppFileManager.shared.fontAssets?.url)!/*FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!*/
