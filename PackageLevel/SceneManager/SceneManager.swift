...
class SceneManager  : NSObject, SceneComposable , TemplateObserversProtocol , ActionStateObserversProtocol, PlayerControlsReadObservableProtocol  {
...
//        MVertexDescriptorLibrary.cleanUp()
    ...
    func canRenderWatermark(_ canRender : Bool) {
...
class ThumbManager : SceneComposable {
...
//        parent._currentTime = smartCurrentTime // globalCurrentTime // - parent.mStartTime
    ...
    func updatePageThumb(pageModel:PageInfo,currentTime:Float = 0.0, size: CGSize = CGSize(width: 300, height: 300)) async{
        guard let templateHandler = self.templateHandler else {
            logger.printLog("template handler nil")
            return }
...
class TextureCache {
    ...
    var fetchIdealSize : CGSize
    var enhancePageSize : Bool = false
    var checkInCache : Bool = true
    var resourceProvider: TextureResourceProvider
    var logger: PackageLogger
