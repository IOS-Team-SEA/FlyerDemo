...
enum SceneOfflineRenderError : Error {
...
class OfflineSceneRenderer : SceneComposable  {
    ...
    enum OfflineRenderingState : Equatable {
        static func == (lhs: OfflineSceneRenderer.OfflineRenderingState, rhs: OfflineSceneRenderer.OfflineRenderingState) -> Bool {
            lhs.value == rhs.value
        }
    ...
    var offlineRenderState : ((OfflineRenderingState)->())?
    ...
    init(settings: ExportSettings , fileHandler : ExportFileHandler , isUserSubscribed : Bool, logger: PackageLogger, resourceProvider: TextureResourceProvider){
        self.logger = logger
