...
protocol PlayerControlsReadObservableProtocol {
...
class TimeLoopHnadler : DrawCallListener , TimeLooperListener ,PlayerControlsReadObservableProtocol, ObservableObject {
 
    func setCurrentTime(_ time: Float) {
        self.currentTime = time.roundToDecimal(2)
    }
    
    var isManualScrolling: Bool = true
    ...
    @Published var timeLengthDuration: TimeInterval = .zero
...
class AnimationTimeLoopHnadler : DrawCallListener   {
    ...
    @Published  var animLoopState : AnimationLoopState = .Stop
