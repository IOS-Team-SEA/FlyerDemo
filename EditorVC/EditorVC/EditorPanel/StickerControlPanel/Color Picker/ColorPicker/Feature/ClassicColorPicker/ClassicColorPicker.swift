...
public class ClassicColorPicker: UIControl, AnyColorPicker {
    public var title: String = "Classic"
    ...
    public override init(frame: CGRect) {
        ...
        hueSlider.addAction(UIAction { [weak self] _ in
            let location = self!.colorView.location(by: self!.color)!
            // hueをセットしてから色を取り出す
            self?.colorView.hue = self!.hueSlider.slider.value
            self?.color = self!.colorView.color(at: location)
