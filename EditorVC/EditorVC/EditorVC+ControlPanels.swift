        ...
        if let engine = engine{
            if let currentModel = engine.templateHandler.currentModel{
                if currentModel is StickerInfo{
                    hostingerController?.rootView = AnyView(StickerContainer(currentModel: engine.templateHandler.currentStickerModel!, currentActionModel: engine.templateHandler.currentActionState, delegate: self).environment(\.sizeCategory, .medium))

                }
                else if currentModel is TextInfo{
                    hostingerController?.rootView = AnyView(TextContainer(currentModel: engine.templateHandler.currentTextModel!, currentActionModel: engine.templateHandler.currentActionState, delegate: self).environmentObject(viewModel.dsStore).environment(\.sizeCategory, .medium))
//                    engine.undoRedoManager?.observeTextModel( currentModel: engine.templateHandler.currentTextModel!, actionStates: engine.templateHandler.currentActionState)
                    ...
                    hostingerController?.rootView = AnyView(ParentContainer(currentModel: engine.templateHandler.currentParentModel!, actionStates: engine.templateHandler.currentActionState, delegate: self, /*ratioSize: engine.templateHandler.currentTemplateInfo?.ratioSize ?? CGSize(width: 1, height: 1)*/ ratioInfo: Binding(
                        get: {
...
//                            engine.templateHandler.currentPageModel?.thumbImage = newValue
                   ...
                   // engine.undoRedoManager?.observeParentModel(currentModel: engine.templateHandler.currentParentModel!, actionStates: engine.templateHandler.currentActionState)
                    ...
                    let pageView = PageContainer(currentModel: engine.templateHandler.currentPageModel!, actionStates: engine.templateHandler.currentActionState, delegate: self, /*ratioSize: engine.templateHandler.currentTemplateInfo?.ratioSize ?? CGSize(width: 1, height: 1)*/ ratioInfo: Binding(
                        get: {
